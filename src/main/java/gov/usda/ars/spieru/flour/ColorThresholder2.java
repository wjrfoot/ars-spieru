 ...
 */
public class ColorThresholder2 extends PlugInFrame implements PlugIn, Measurements,
    ...
    private int[] parseArg(String varName, String mode, String params) {
        String[] chunks = params.split(" ");
        for (int idx = 0; idx < chunks.length; idx++) {
            if (chunks[idx].startsWith(varName)) {
                String[] items = chunks[idx].substring(2).split("-");
                int[] rtn = new int[2];
                rtn[0] = Integer.parseInt(items[0]);
                rtn[1] = Integer.parseInt(items[1]);
                return rtn;
            }
        }
        throw new IllegalArgumentException(varName + " is not valid for mode " + mode);
    }
    
    public ColorThresholder2(ImagePlus ip, String params) {

        super("Threshold Color");
        
        WindowManager.setTempCurrentImage(ip);

        for (int idx = 0; idx < colorSpaces.length; idx++) {
            if (params.contains(colorSpaces[idx])) {
                colorSpace  = idx;
                break;
            }
        }
        ...
        switch(colorSpace) {
            case HSB : ;
                param1 = parseArg("H", colorSpaces[colorSpace], params);
                param2 = parseArg("S", colorSpaces[colorSpace], params);
                param3 = parseArg("B", colorSpaces[colorSpace], params);
                break;
            case RGB : ;
                param1 = parseArg("R", colorSpaces[colorSpace], params);
                param2 = parseArg("G", colorSpaces[colorSpace], params);
                param3 = parseArg("B", colorSpaces[colorSpace], params);
                break;
            case LAB : ;
                param1 = parseArg("L", colorSpaces[colorSpace], params);
                param2 = parseArg("A", colorSpaces[colorSpace], params);
                param3 = parseArg("B", colorSpaces[colorSpace], params);
                break;
            case YUV : ;
                param1 = parseArg("Y", colorSpaces[colorSpace], params);
                param2 = parseArg("U", colorSpaces[colorSpace], params);
                param3 = parseArg("V", colorSpaces[colorSpace], params);
                break;
        }
        
        boolean passFlg[] = new boolean[3];
        passFlg[0] = !params.contains("nopass1");
        passFlg[1] = !params.contains("nopass2");
        passFlg[2] = !params.contains("nopass3");
        bandPassH = new Checkbox();
        bandPassH.setState(passFlg[0]);
        bandPassS = new Checkbox();
        bandPassS.setState(passFlg[1]);
        bandPassB = new Checkbox();
        bandPassB.setState(passFlg[2]);
        
        for (int idx = 0; idx < modes.length; idx++) {
            if (params.contains(modes[idx])) {
                mode  = idx;
            }
        }
        
        minSlider = new Scrollbar();
        minSlider.setValue(param1[0]);
        adjustMinHue(param1[0]);
        maxSlider = new Scrollbar();
        maxSlider.setValue(param1[1]);
        adjustMaxHue(param1[1]);
        minSlider2 = new Scrollbar();
        minSlider2.setValue(param2[0]);
        adjustMinSat(param2[0]);
        maxSlider2 = new Scrollbar();
        maxSlider2.setValue(param2[1]);
        adjustMaxSat(param2[1]);
        minSlider3 = new Scrollbar();
        minSlider3.setValue(param3[0]);
        adjustMinBri(param3[0]);
        maxSlider3 = new Scrollbar();
        maxSlider3.setValue(param3[1]);
        adjustMaxBri(param3[1]);

        darkBackground = new Checkbox();
        darkBackground.setState(params.contains("dark"));
...
//        reset(ip);
    ...
    private static ImageProcessor RGBtoLab(ImageProcessor ip) {
        ...
        byte[] a = new byte[n];
        byte[] b = new byte[n];
        ColorThresholder2.getLab(ip, L, a, b);
